    ...
    <View>
      ...
      <View className='flex-row flex-wrap gap-4'>
        {nutrition.map((item, index) => (
          <View key={index} className='bg-surface-tertiary p-4 rounded-lg w-[48%]'>
            <Text className='text-sm text-text-secondary font-medium mb-1'>{item.label}</Text>
            <View className='flex-row items-baseline'>
              <Text className='text-2xl font-bold text-primary'>{item.value}</Text>
              <Text className='text-sm text-text-secondary ml-1'>{item.unit}</Text>
            </View>
          </View>
        ))}
      </View>
