...
export default function RecipeCard({ recipe, isGridView }) {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <AntDesignIcon key={`full-${i}`} name="star" color={"gold"} size={14} />
      );
    }
  ...
  return (
    <TouchableOpacity
      onPress={handlePress}
      className={`${
        isGridView ? "w-1/2" : "w-full"
      } shadow-sm shadow-neutral-300`}
    >
      <View
        className={`${
          isGridView ? "flex-col" : "flex-row"
        } relative bg-white m-2 rounded-lg`}
      >
        {/* Recipe image  */}
        <View className={`${isGridView ? "h-44" : "w-1/3"}  `}>
          <Image
            className={`${
              isGridView ? "h-44 rounded-t-lg" : "h-32 rounded-tl-lg"
            } bg-gray-400 `}
        ...
        <View className={`${isGridView ? "w-full" : "w-2/3"} p-4  gap-2`}>
          <View className={`${isGridView ? "flex-col" : "flex-row"} gap-2`}>
            ...
            {!isGridView && (
              <View>
                <Text className="bg-green-500 text-green-50 px-2 py-1 rounded-lg font-semibold">
                  {recipe.level}
                </Text>
              </View>
            )}
          ...
          <View className="flex-row justify-between">
            ...
            {/* Category  */}
            <Text className="bg-cyan-100 text-cyan-900 px-2 py-1 rounded-lg">
              {recipe.category}
            </Text>
          </View>
          {/* Dietary needs  */}
          <View className="flex-row flex-wrap gap-2">
            {recipe.dietary_needs?.map((need, index) => (
              <Text
                key={index}
                className="bg-blue-50 text-blue-900 px-2 py-1 rounded-lg"
              >
                {need}
              </Text>
            ))}
